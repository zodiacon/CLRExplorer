...
bool SyncBlocksView::Init(CListViewCtrl& lv, IGenericListView* glv) {
	...
	lv.InsertColumn(2, L"Sync Block", LVCFMT_RIGHT, 120);
	lv.InsertColumn(3, L"Monitors Held", LVCFMT_CENTER, 90);
	lv.InsertColumn(4, L"Owner Thread", LVCFMT_CENTER, 120);
	lv.InsertColumn(5, L"Owner Thread ID", LVCFMT_LEFT, 150);
	lv.InsertColumn(6, L"Recursion", LVCFMT_CENTER, 80);
	lv.InsertColumn(7, L"COM Flags", LVCFMT_RIGHT, 80);
...
CString SyncBlocksView::GetItemText(int row, int col) {
	...
	switch (col) {
		...
		case 2: return FormatHelper::ToHex(item.SyncBlockPointer);
		case 3: return FormatHelper::ToDec(item.MonitorHeld);
		case 4: return item.HoldingThread == 0 ? L"" : FormatHelper::ToHex(item.HoldingThread);
		case 5:
			if (item.HoldingThread) {
				...
				return L"";
		case 6: return item.HoldingThread == 0 ? L"" : FormatHelper::ToDec(item.Recursion);
		case 7: return FormatHelper::ToHex(item.COMFlags);
	}
